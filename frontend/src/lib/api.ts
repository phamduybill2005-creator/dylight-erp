...
export const api = {
  ...
  progressHistory: (projectId: number) =>
    request<ProgressHistory>(`/projects/${projectId}/progress-history`),
  ...
  evaluationsStarOverview: () =>
    request<StarOverviewRow[]>("/evaluations/star-overview"),
  ...
  // --- Timesheet (giờ làm thực tế theo dự án/ngày) ---
  timesheets: (params?: { from?: string; to?: string; userId?: number; projectId?: number }) => {
  ...
  clearWorkerHours: (payload: { project_id: number; user_id: number; project_item_id?: number | null }) =>
    request<{ deleted: number }>("/timesheets/clear-worker", { method: "POST", body: JSON.stringify(payload) }),
