...
def create_project(
    ...
    # Cho phép TẤT CẢ nhân viên trong công ty được tạo dự án
...
def delete_project(
    ...
    """Xoá dự án (Chuyển vào Thùng rác / Soft-delete để có thể khôi phục)."""
    p = db.get(Project, project_id)
    if not p or p.company_id != current.company_id or p.is_deleted:
        raise HTTPException(404, "Không tìm thấy dự án.")
    
    if not _can_manage(db, p, current) and not (_is_director(current) or _is_senior_manager(db, current)):
        raise HTTPException(403, "Bạn không có quyền xoá dự án này.")
